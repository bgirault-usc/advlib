 ...
 * @param {Object} advertiserData The object containing all parsed data.
 */
function processTiles(advertiserData) {
  var norble = {};
  var data = advertiserData.manufacturerSpecificData.data;
  var frameLength = parseInt(data.substr(2,2), 16) & 0x1f;

  norble.cyclicCount = parseInt(data.substr(2,1), 16) >> 1;
  norble.batteryPercentage = toBatteryPercentage(data.substr(4,2), false);
  norble.nearest = [];

  for(nearestIndex = 3; nearestIndex < (frameLength + 2); nearestIndex += 3) {
    var instanceId = data.substr(nearestIndex * 2 + 2, 4);
    var rssiEddystone = parseInt(data.substr(nearestIndex * 2, 2), 16);
    var isEddystone = (rssiEddystone >> 7) == 1;
    var rssi = (rssiEddystone & 0x3f) - 92;
    norble.nearest.push( { instanceId: instanceId, rssi: rssi, isEddystone: isEddystone } );
  }
